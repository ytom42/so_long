...
/*   Updated: 2021/12/25 17:20:34 by ytomiyos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_img(t_all *s)
{
	...
	while (window_y < s->screen_h)
	{
		...
		while (window_x < s->screen_w)
		{
			...
			s->tex_x = window_x % 64;
			s->tex_y = window_y % 64;
			dst = s->map[map_y][map_x];
			if (dst == '0' || dst == '1' || dst == 'P' || dst == 'C' || dst == 'E')
			...
			if (dst == '1')
				...
				mlx_pixel_put_of_tex(s, window_x, window_y, &s->texs.collectible);
			else if (dst == 'E')
